...
const GlassesFinderWizard = () => {
  ...
  const generateAIInsights = async () => {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyBwPLk8BhFMmAMWSAlKC3DOgPMNqL6BJXA`, {
        method: "POST",
        headers: {
...
CRITICAL: You must provide exactly 3 glasses recommendations every time. Vary recommendations based on their actual answers. Make it personal and specific. ALWAYS EXACTLY 3 GLASSES.`
      ...
      console.log("API call failed, using fallback recommendations:", error);
      setLoadingProgress(100);
  ...
  const renderOpticiansList = () => {
    const mockStores = [
      ...
      { 
        ...
        coordinates: { lat: 40.4378, lng: -3.6888 }
      }
    ];
  ...
  const updateAnswer = (questionNum: number, value: any) => {
    setAnswers(prev => ({
      ...prev,
      [questionNum]: value
    }));
  ...
  const isAnswered = () => {
    ...
    const answer = (answers as any)[currentStep];
  ...
  const getSectionInsights = (sectionName: string, answers: any) => {
    const insights = {
      "Your Current Setup": {
    ...
    return (
      <div className="text-center space-y-6">
        ...
        <div className="space-y-3">
          <h2 className="text-2xl font-bold text-gray-800">{(completedSectionData as any).title}</h2>
          <p className="text-lg text-gray-600 max-w-md mx-auto">{(completedSectionData as any).insight}</p>
        </div>
  ...
  const renderQuestion = () => {
    ...
    if (q.type === 'slider' && (answers as any)[currentStep] === undefined) {
    ...
    return (
      <div className="space-y-6">
        ...
        {q.type === 'slider' ? (
          <div className="space-y-6">
            ...
            <div className="relative px-4">
              <input
                ...
                min="0"
                max="100"
                value={(answers as any)[currentStep] || q.sliderDefault}
                ...
                style={{
                  background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${(answers as any)[currentStep] || q.sliderDefault}%, #e5e7eb ${(answers as any)[currentStep] || q.sliderDefault}%, #e5e7eb 100%)`
                }}
              />
            </div>
            <div className="text-center text-sm text-gray-500">
              Current preference: {
                ((answers as any)[currentStep] || q.sliderDefault) < 25 ? 'Strongly price focused' : 
                ((answers as any)[currentStep] || q.sliderDefault) < 50 ? 'Somewhat price focused' :
                ((answers as any)[currentStep] || q.sliderDefault) < 75 ? 'Somewhat style focused' : 
          ...
          <div className="space-y-4">
            <textarea
              value={(answers as any)[currentStep] || ''}
          ...
          <div className="space-y-3">
            {q.options!.map((option, index) => (
              <button
                ...
                onClick={() => {
                  if (q.type === 'multiple') {
                    const current = (answers as any)[currentStep] || [];
                ...
                className={`w-full p-4 text-left rounded-xl border-2 transition-all duration-200 hover:border-blue-400 hover:bg-blue-50 ${
                  q.type === 'multiple'
                    ? ((answers as any)[currentStep] || []).includes(option)
                      ? 'border-blue-500 bg-blue-100 text-blue-800'
                      : 'border-gray-200 bg-white'
                    : (answers as any)[currentStep] === option
                      ? 'border-blue-500 bg-blue-100 text-blue-800'
                      : 'border-gray-200 bg-white'
                }`}
              >
      ...
      return (
        <div className="text-center space-y-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-lg text-gray-600">Analyzing your responses to provide personalized recommendations...</p>
          </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl p-8 border border-blue-100 shadow-lg">
          <div className="flex items-start space-x-4 mb-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600">AI-powered analysis based on your lifestyle and vision needs</p>
            </div>
          </div>
